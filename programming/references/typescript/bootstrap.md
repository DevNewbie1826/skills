...
> Read this when starting a TypeScript project or configuring its runtime, package manager, and tooling.
...
Bun is the default, but adapt commands to Node/npm/pnpm/yarn when the project standardizes on them; the contract is the config, not the runner.
...
## Package manager — pnpm as the primary Node fallback

Prefer pnpm for new Node projects; npm or yarn are valid when project conventions or platform support require them.
...
# only inside the project being migrated, after confirming via git status (or a snapshot) that no uncommitted work depends on the current install state.
...
`noEmit: true` because the selected runner and build tool handle compilation. The `tsc` command becomes a typechecker only.

## Quick-start: Bun + Hono backend

The scaffolder defaults to the Bun variant:

```bash
node ../../scripts/typescript/new-project.ts my-api --runtime bun
cd my-api
bun install
bun run dev
```

For a Node deployment, select the Node variant. It emits npm scripts, `tsx`, Node types, and the Hono Node server adapter:
...
For greenfield backend services, use the Hono lane with Bun as the primary runner or equivalent Node and package-manager commands.
