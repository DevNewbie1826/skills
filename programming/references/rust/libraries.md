...
> Read this when selecting Rust libraries or an implementation lane for a new concern.
...
Generalize the pattern to your own domains (see `type-state.md`).
...
For lock-free or atomic-heavy code (channels, refcounts, hazard pointers). See `concurrency.md` for the full pattern.
