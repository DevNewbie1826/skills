...
> Read this when writing or reviewing Python source, packages, scripts, or project configuration.
...
Or use plain pip / pipx / python3 -m venv if uv is unavailable.
...
For manual setup: `uv init --app myproject`, or use plain pip / pipx / python3 -m venv if uv is unavailable; then load `pyproject-strict.md`.

## PEP 723 — inline script metadata (mandatory for ALL scripts)

Every `.py` script — even throwaway — MUST use PEP 723 inline metadata with the `# ─── How to run ───` comment block. The metadata is the primary environment spec; use plain pip / pipx / python3 -m venv if uv is unavailable. A script without the usage comment block is incomplete.
...
From the skill directory, run `python3 scripts/python/check-no-excuse-rules.py <changed-paths>` after every edit session. The checker uses only Python's standard library; use the platform's compatible Python launcher if `python3` is unavailable.
