...
func TestMain(m *testing.M) {
    goleak.VerifyTestMain(m,
        goleak.IgnoreTopFunction("your/module.(*Registry).backgroundWorker"),
    )
}
```

Replace the placeholder with the exact fully qualified function reported for a known long-lived worker. One line at the top of every package that spawns goroutines catches the bug class the race detector cannot.
