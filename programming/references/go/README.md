...
> Read this when writing or reviewing Go source, modules, service definitions, or Go-adjacent interface specifications.
...
| HTTP framework lane | Choose `gin-gonic/gin` for its full framework ecosystem, `go-chi/chi` for minimal `net/http` composition, or `connectrpc/connect-go` for RPC; see `libraries.md` for trade-offs | Pick a framework by service constraints, not habit |
...
- **HTTP backend (framework lane)** → `backend-stack.md` — service skeleton, middleware ordering, SSE/streaming with `http.Flusher`, structured logging, and graceful shutdown.
...
The references contain the recipes. **Read them before writing code. Re-read them when project constraints change.** The post-write architectural review loop is non-negotiable.
