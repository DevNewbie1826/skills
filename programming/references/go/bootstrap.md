...
> Read this when starting a Go project or configuring its layout, tooling, task runner, or CI.

What every new Go project gets in the first 60 seconds. The `scripts/go/new-project.py` script does all of this — this document explains *what* it produces and *why*.
...
tasks:
  ...
  migrate:down:
    cmds:
      # migrations down/rollback only against a disposable development database; never against a shared or production database without explicit user approval and a verified backup/restore path.
...
The skill's `scripts/go/new-project.py` writes this file with project-specific values filled in.
