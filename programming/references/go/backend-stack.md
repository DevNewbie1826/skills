# HTTP Backend Stack — Go Framework Lane

> Read this when implementing a Go HTTP service with routing, middleware, streaming, request logging, and graceful shutdown.

This production HTTP service skeleton documents one established Go framework lane, including SSE streaming, WebSocket upgrades, request logging, and hot-reload configuration. Read `libraries.md` to choose this lane or an alternative based on the service's actual constraints.
...
A production proxy workload streams provider-compatible SSE for hundreds of concurrent clients. The pattern:
