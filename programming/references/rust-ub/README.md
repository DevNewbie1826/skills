...
> Read this when auditing Rust `unsafe`, raw pointers, FFI, manual `Send`/`Sync`, or custom lock-free code.
...
1. **Miri first, always.** Before reading a single line of `unsafe`, run Miri. Before proposing a fix, run Miri. After applying a fix, run Miri. Miri is the primary evidence.
