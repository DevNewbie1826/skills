...
> Read this when adding or changing log lines, logger setup, service entry points, or boundary error handling.
...
This reference is deliberately stack-agnostic. It never tells you which logging library to use; the language references describe implementation options, and the project's existing practice overrides everything (Rule 0).

---

## Rule 0 — Discover the project's practice before emitting anything

**BEFORE adding a single log line, find out how this project logs.** Search for logger initialization, a wrapper module, prior art in sibling files, and project guidance documents.
...
Serialization contracts (reserved field names, argument order, the key an Error must be passed under) are project knowledge. Discover the contract from the logger configuration and existing call sites; record what you found in the project's guidance or change notes.
