...
Read this when the user asks for a reflective report based on a user-provided taste profile.
...
Read the user-provided taste profile and project history, then synthesise — don't just reformat the tables. Look for the patterns *across* entries that the designer might not see themselves.
...
- **Reads from:** a user-provided taste profile and its project history (via `design-memory`)
