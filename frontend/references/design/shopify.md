...
1. Reference specific color names and hex codes from this document
2. Remember: this is a DARK-FIRST design — light surfaces are the exception, not the rule
3. Display text should always feel feather-light (weight 330-400) — if it looks heavy, reduce the weight
4. Neon Green (#36F4A4) is precious — use sparingly for focus and accent only
5. The dark surface hierarchy (black → deep teal → dark forest → forest) creates subtle depth
6. Shadows are multi-layered — a single `box-shadow` value won't capture the Shopify card feel
7. `ss03` OpenType feature must be active on all text for typographic consistency
