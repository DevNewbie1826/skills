...
2. Use natural language descriptions ("warm cream canvas," "four-tier green system") alongside exact values
3. Preserve the 50px pill + `scale(0.95)` active state universally
4. Check that greens are mapped to their correct role (Green Accent for CTA, Starbucks Green for heading, House Green for band)
5. Don't introduce gradients — the system is color-block
6. Keep SoDoSans tracking at `-0.01em` / `-0.16px` across the board
