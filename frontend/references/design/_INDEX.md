...
Read this when selecting a focused design reference or Layer A skill.

Most Layer A and Layer B references live flat in this directory; focused task domains live in named subdirectories. Three layers:
...
1. **At most one *style* skill at a time** (`taste-skill`, `cinematic-taste-skill`, `minimalist-skill`, `brutalist-skill`, `soft-skill`, `redesign-skill`, `image-to-code-skill`). They encode opposite philosophies - picking two creates contradictions.
2. **`output-skill.md` and `mockup-export-skill.md` stack on top of any style skill.** They add discipline and output format, not visual direction.
3. **`image-to-code-skill.md` pairs with one image-concept skill** for the full flow.
4. **Image-concept skills are image-only.** Do not load them when the user actually wants code.

---

## Supporting workflows and tooling

| File | Purpose | Load when |
|---|---|---|
| [`clone-from-url.md`](clone-from-url.md) | Extract a live site's runtime design system into reusable primitives. | A live URL is the visual reference or the user requests a clone. |
| [`lazyweb.md`](lazyweb.md) | Research shipped product screens by domain and surface. | Greenfield research needs real-product evidence and the service is available; otherwise record the named skip in `DESIGN.md`. |
| [`react-dev-tooling-skill.md`](react-dev-tooling-skill.md) | Optional dev-only source inspection, render diagnostics, and static analysis. | A React project can benefit from those tools and its dependency policy permits them; use equivalent tooling for other stacks. |

---

## Layer B — Design Systems (70)

Most Layer B files are materialized from [VoltAgent/awesome-design-md](https://github.com/VoltAgent/awesome-design-md), based on an open `DESIGN.md` format. Each file captures one website's complete visual language: color palette, typography, components, layout principles, depth, do/don't, responsive behavior, and an agent prompt guide. Project-local synthesis guides such as `aside.md`, `lazyweb.md`, and `clone-from-url.md` are documented in the [attribution notice](../../ATTRIBUTION.md).
...
- **"Editorial / paper-like / readable"** → `notion.md`, `wired.md`, and [`cl&#97;ude.md`](cl%61ude.md)
