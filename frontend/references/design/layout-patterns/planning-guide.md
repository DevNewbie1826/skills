...
description: Pre-design entry point for choosing and composing layout patterns.
---
...
This layout collection has two modes:

- **After a problem appears:** find a named pattern in `CATALOG.md` and port its HTML/CSS contract.
- **Before a problem appears:** start with the screen type, answer the layout brief, then compose patterns from the [Layout Recipes](../layout-recipes/index.md).
- **When content needs to become a webpage:** use the [Webpage generation workflow](guides/webpage-generation-workflow.md) to classify the use case, make the content-to-layout match, run harmony evaluation, create the reference image, and record the implementation handoff before writing product CSS.
...
9. If visual direction is needed, create a reference image from the approved layout brief and keep the image outside reusable pattern CSS.
...
- [Recipe index](../layout-recipes/index.md): screen-type compositions built from reusable patterns.
...
- A landing page, product homepage, personal site, and campaign page may all start from the [Homepage](../layout-recipes/homepage.md) recipe, then diverge by section jobs and visual direction.
