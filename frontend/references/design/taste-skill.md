...
1. **Image-generation tool first, when available.** Use an available image-generation model or design-mockup tool to create section-specific assets: hero photography, product shots, texture backgrounds, and mood images at the right aspect ratio. If none is available, continue to the real-image fallback below and state which fallback was used.
