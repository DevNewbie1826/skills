Read this when a fix needs validation through the same real surface that exposed the bug.
...
|---|---|
| **CLI tool** | Use a terminal multiplexer such as `tmux` when available, otherwise a regular terminal. Run the actual command end-to-end and capture output. Include exit code, stdout, stderr, and side-effect checks. |
| **HTTP API** | Start the real server, hit endpoints with `curl` or `httpie`, inspect response status, body, and headers. Hit the specific endpoint that reproduced the bug. If there is auth, use real auth. |
| **Browser-served web app** | Drive a real browser with [Playwright CLI](../tools/playwright-cli.md) when available. If it is unavailable, use another browser-driving capability or manually drive a real browser and record the limitation. Capture screenshot, DOM, and network evidence. **Do not substitute with curl** — browsers have state that curl does not have. |
| **Model or tool-calling pipeline** | Run the same user prompt that originally failed. Capture the full turn: tool calls, messages, and usage counters. **Confirm non-zero usage** when usage is expected (zero can indicate a silent failure). |
...
| **MCP server** | Invoke the tool through its actual protocol client when available, or a protocol-capable CLI. Do not rely only on an HTTP probe endpoint; the MCP handshake itself can contain the bug. |
