Read this when evidence points to a cause and you need to prove the mechanism before making the smallest safe fix.
...
Not "the value looks consistent with" — the value is exactly the value the hypothesis predicted. If your hypothesis was "baseUrl is `https://provider.example` despite `SERVICE_BASE_URL` being set to a proxy", capture that exact URL in the debugger at the moment of the HTTP call.

### 2. Reproducible

Running the repro a second time yields the same observation. Flaky repros mean you haven't isolated the cause; you've isolated a symptom that sometimes appears when the cause does. Investigate at most three further rounds on the flaky repro; then proceed with [partial-runtime-evidence.md](partial-runtime-evidence.md) and name the residual uncertainty.
...
- **Test name reads like a bug report.** `test_refinement_turn_returns_empty_content_when_upstream_returns_401` is good. `test_bug_fix` is not.
...
If diagnosis burned extra rounds *because state was invisible* — no log line told you which branch ran, what the value was, whether the fallback engaged — that invisibility is a defect adjacent to the bug, and this is the one moment where adding a log line is NOT "just in case": this session is the evidence that the line earns its place. Ship the fix with that line: choose its level for its consumer, place it at the decision point, put data in fields rather than interpolation, and use the project's designated logger. If the programming skill is installed, its logging reference goes deeper; otherwise apply structured, consumer-routed logging practices. If the project deliberately does not log, respect that and skip this step.
