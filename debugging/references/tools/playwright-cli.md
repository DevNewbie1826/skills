Read this when a browser-served UI bug needs reproduction or QA through a real browser.

# Playwright CLI — Browser QA That Actually Drives a Browser

**https://playwright.dev/ · https://github.com/microsoft/playwright**

Playwright drives a real browser with a rendering engine, JavaScript execution, cookies, service workers, and viewport state. Use it when available for browser-served UI bugs. If it is unavailable, use another browser-driving capability or manually drive a real browser and capture equivalent evidence; state the limitation. Do not substitute an HTTP client for browser behavior.

**Phase 8 requires real-browser QA.** Curl cannot catch CSS that breaks at specific viewport widths, hydration mismatches, client-side router bugs, cookie/session interactions, service-worker caching, or JavaScript-triggered navigations.
...
# Delete only report, results, and trace paths this session created and journaled; verify ownership before rm.
rm -rf <journaled-playwright-report> <journaled-test-results> <journaled-trace-zip>
