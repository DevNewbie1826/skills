Read this when a binary or network bug needs a reproducible scripted interaction and pwntools is available.

# pwntools — Scripted Binary / Network Interaction

**https://docs.pwntools.com/en/stable/ · https://github.com/Gallopsled/pwntools**

pwntools is a Python framework for building reproducible interactions with binaries and network services. Use it when installed. If it is unavailable, write an equivalent deterministic harness with Python's `subprocess` or `socket` facilities (or an available language/runtime) and record which pwntools-only observations were unavailable. It is useful whenever you need:
...
# Kill by the PID recorded in this session's journal at spawn; use a pattern-kill only after `pgrep -af './target'` confirms it is this session's process.
kill <journaled-target-pid> 2>/dev/null || true
