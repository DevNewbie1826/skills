Read this when a native binary needs source-like decompilation and Ghidra is available.

# Ghidra — Decompile Binaries Into Readable C

**https://github.com/NationalSecurityAgency/ghidra**

Ghidra is an open-source reverse-engineering suite whose decompiler turns machine code back into readable C. Use it for third-party closed-source libraries, malware analysis, vendored binaries whose behavior contradicts documentation, CTF challenges, firmware, or any time you need to read compiled code. If Ghidra is unavailable, use an installed decompiler or disassembler, preserve the same evidence trail, and state the loss of decompiler fidelity.
...
# Kill by the PID recorded in this session's journal at spawn; use a pattern-kill only after `pgrep -af analyzeHeadless` confirms it is this session's process.
kill <journaled-analyzeHeadless-pid> 2>/dev/null || true
```
