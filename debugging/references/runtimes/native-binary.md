Read this when debugging a native binary without trustworthy source, including stripped builds, third-party libraries, firmware, or reverse-engineering targets.

# Native Binary Debugging (No Source / Reverse Engineering)

For binaries where you do not have trustworthy source, the workflow is specific; doing it out of order wastes days. If a named tool below is unavailable, use the closest installed decompiler, debugger, disassembler, or scripting facility and explicitly record what evidence it could not provide.

This reference **coordinates** the triage and dynamic work. The heavy tools each have their own reference:
- **Static decompilation** → [Ghidra, if available](../tools/ghidra.md), otherwise an installed decompiler or disassembler
- **Interactive debugging** → [pwndbg, if available](../tools/pwndbg.md), otherwise GDB or lldb
- **Scripted interaction / exploitation** → [pwntools, if available](../tools/pwntools.md), otherwise a deterministic subprocess or socket script
...
# 6. Cleanup — RESTORE the exact original proxy and certificate state from the journal:
# Restore the journaled pre-session web and secure proxy settings for "$SERVICE"; do not force either proxy off.
<journaled-networksetup-proxy-restore-commands>
# Restore the journaled prior certificate state; delete only the certificate this session added.
<journaled-security-certificate-restore-command>
```
...
**Open [tools/ghidra.md](../tools/ghidra.md) before launching Ghidra, if it is available** — the import, analysis, and decompile workflow is not obvious. If it is unavailable, use the installed decompiler or disassembler and record that its output may be less readable.

Ghidra's decompiler turns machine code into readable-ish C. Stay in the Decompiler view; drop to Listing (disassembly) only when the decompiler punts.
...
**Open [tools/pwndbg.md](../tools/pwndbg.md) before launching GDB if pwndbg is available.** Otherwise use GDB or lldb's standard register, stack, and disassembly commands; the same observations remain required.
...
**Open [tools/pwntools.md](../tools/pwntools.md) if pwntools is available.** Otherwise build the same deterministic repro with available process or socket facilities.
...
# Kill by the PID recorded in this session's journal at spawn; use a pattern-kill only after `pgrep -af gdb` confirms it is this session's process.
kill <journaled-gdb-pid> 2>/dev/null || true
pkill -f 'lldb' || true
...
# Delete only core-dump paths this session created and journaled; verify ownership before rm.
rm -f <journaled-core-dump-path>
