Read this when debugging Go services, goroutines, races, memory use, or stuck work.

# Go Debugging

Covers goroutines, `dlv` (Delve), `pprof`, and the race detector. Prefer `dlv` when it is available; otherwise use Go runtime stack dumps, `go tool pprof`, focused logging, or another debugger that understands Go, and record the limitation.
...
# Delve installed?
command -v dlv && dlv version || echo 'dlv unavailable; use runtime dumps or another Go-aware debugger' 
...
Go's gc compiler emits DWARF, but plain GDB barely understands goroutines. **Use `dlv` when available.** It understands goroutine state and interface values; if it is unavailable, prefer runtime goroutine dumps and profiling over plain GDB.
...
```bash
# Kill dlv sessions
# Kill by the PID recorded in this session's journal at spawn; use a pattern-kill only after `pgrep -af dlv` confirms it is this session's process.
kill <journaled-dlv-pid> 2>/dev/null || true
...
# Reverse only the journaled debug hunk. For a whole-file restore,
