Read this when debugging Node.js, TypeScript runners, Bun, or Deno source processes.

# Node.js / tsx / ts-node / Bun / Deno Debugging

Covers Node 18+, tsx, ts-node, Bun, and Deno. Launch recipes, inspector protocol usage, the `node inspect` CLI, and the tsx source-map silent failure. If a named inspector, terminal multiplexer, or runtime-specific tool is unavailable, use the closest available debugger or standard logging and record the limitation; do not make tool availability a prerequisite for evidence.
...
| **Inspector-compatible GUI** | CLI not required, faster iteration | Not usable if the user specifically asked for CLI |
...
**Critical**: Bun uses WebKit Inspector Protocol, not V8. A V8-only inspector cannot connect directly. Use Bun's inspector page or another compatible inspector client if available.
...
```js
// Session or model-call state; adapt object paths to the application.
exec('this.session.state.messages.length')
exec('this.session.state.messages.map(m => m.role)')
exec('JSON.stringify(this.session.state.messages.at(-1)).substring(0, 500)')
exec('this.session.state.messages.at(-1).errorMessage')      // silent-error sentinel
exec('this.session.state.messages.at(-1).stopReason')
exec('JSON.stringify(this.session.state.usage)')             // undefined / all-zero = failed call
exec('this.session.state.model.baseUrl')                      // catch hardcoded vs env-var

// Env / config at runtime
exec('process.env.RELEVANT_VAR')
exec('Object.keys(process.env).filter(k => k.startsWith("RELEVANT_PROVIDER"))')
exec('this.config')
...
| HTTP 200 + response in <1s for a remote model call | Too fast for the expected operation; something short-circuited |
...
## Optional terminal-multiplexer layout

If `tmux` is available, it is a convenient way to keep the inspected process and debugger client separate:
...
# Interactive debugger client
tmux new-session -d -s debug-client -c "$PWD"
tmux send-keys -t debug-client 'node inspect 127.0.0.1:9229' Enter

# Non-blocking pane inspection
tmux capture-pane -p -t debug-server -S -50
```

Without a terminal multiplexer, use two ordinary terminal windows for the same commands. Journal the sessions or windows you create and stop them with the matching mechanism at Phase 9.

```bash
# Only if tmux was used:
...
## When to abandon the CLI and switch to an inspector GUI
...
Phrase as a note, not a request: "I can push through with `debugger;` statements in the CLI. If we hit three or more of these in a row, switching to an inspector-compatible GUI would cut cycle time in half — your call."
...
# Reverse only the journaled matching hunk. For a whole-file restore,
# use the pre-change journal snapshot; never use a blanket checkout.

# Kill inspector-attached processes
# Kill by the PID recorded in this session's journal at spawn; use a pattern-kill only after `pgrep -af 'node --inspect'` confirms it is this session's process.
kill <journaled-node-inspector-pid> 2>/dev/null || true
