Read this when debugging CPython applications, tests, async work, or Python web services.

# Python Debugging

Covers CPython 3.9+, pytest, asyncio, Django, and FastAPI. Setup commands, attach mechanisms, state-query patterns, gotchas, and silent-failure signatures. If an optional debugger or profiler is unavailable, use built-in `pdb`, `faulthandler`, `cProfile`, or `tracemalloc` as appropriate and record the limitation.
...
| **debugpy (remote / IDE)** | IDE attach, remote host, containerized process; if unavailable, use `pdb` or application logging. | `python -m debugpy --listen 5678 --wait-for-client script.py`, then attach from a compatible debugger client. |

### Use `ipdb` or `pudb` when installed; otherwise use plain `pdb`
...
Attach from a compatible debugger client. One launch-configuration shape is:
```json
...
When the problem is performance or a hang (not a crash), do not attach pdb — it alters timing. Use a sampling profiler that attaches to the running process when one is installed; otherwise capture stacks with `faulthandler` and use built-in profiling or allocation tracing:
...
# Reverse only the journaled debug hunk. If a whole-file restore is necessary,
...
# Kill by the PID recorded in this session's journal at spawn; use a pattern-kill only after `pgrep -af debugpy` confirms it is this session's process.
kill <journaled-debugpy-pid> 2>/dev/null || true
