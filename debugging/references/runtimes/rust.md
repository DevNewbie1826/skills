Read this when debugging Rust code, panics, async tasks, unsafe code, or release-only behavior.

# Rust Debugging

Covers `cargo`, `tokio`, and panics. You often do not need a debugger: Rust's type system, `dbg!`, and logging resolve many sessions faster than a native debugger. If an optional debugger, task console, or macro-expansion tool is unavailable, use backtraces, tracing, and the closest standard debugger or compiler output, then record the limitation.
...
**Pair with pwndbg when available** for a better native-debugging layout — see [tools/pwndbg.md](../tools/pwndbg.md). Otherwise use standard GDB or lldb inspection commands. Pwndbg works with rust-gdb too.

---

## tokio-console — async task debugging

For tokio-based async apps, use this when it is installed and tasks are stuck or leaking. If it is unavailable, add temporary `tracing` instrumentation and inspect task lifecycle logs instead.
...
Use it when installed. If it is unavailable, inspect compiler diagnostics and macro documentation, and state that expanded output was not available.
...
git diff | grep -E 'dbg!\('
# Reverse only the journaled dbg! hunk. For a whole-file restore,
...
# Kill by the PID recorded in this session's journal at spawn; use a pattern-kill only after `pgrep -af '^lldb '` confirms it is this session's process.
kill <journaled-lldb-pid> 2>/dev/null || true
