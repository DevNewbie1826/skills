Read this when a large executable may contain recoverable JavaScript, TypeScript, or Python source rather than only native code.

# Bundled-JS / Embedded-Source Binaries (Bun SEA, Node SEA, Deno compile, pkg, Electron, PyInstaller)

A growing class of "binaries" are not stripped C/C++ at all — they are a runtime VM glued onto a high-level-language bundle. The bundle is **plaintext or trivially-decodable** inside the binary. If a named extractor is unavailable, use another format-aware extractor or byte-level analysis, verify the result, and state the extraction limitation.
...
```bash
# macOS app-bundle layout; set this to the bundle being inspected.
APP_BUNDLE=<path-to-app>.app
ls -la "$APP_BUNDLE/Contents/Resources/app.asar"
npx @electron/asar extract "$APP_BUNDLE/Contents/Resources/app.asar" ./extracted/
# or older:
...
rg -n "provider-model-id|reasoning_effort|api_key" extracted-bundle.js
...
# Delete only the exact `_MEI` directory this session created and journaled; verify ownership before rm.
rm -rf <journaled-extraction-directory>
...
# Verify the extraction directory is gone (use the directory recorded in the journal).
ls -la "${EXTRACTION_DIR:-./extracted}" 2>/dev/null
# Remove it only after journal review confirms nothing important is there.
```
