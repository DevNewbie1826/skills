...
1. **Skip decomposition entirely** — no slice table, no `depends_on`. Confirm it at the scope/scale gate, then dispatch one eval `agent(agent='task')` (per Step 2 delegation discipline).
...
**Eval is the execution engine.** eval agent()/parallel() run ALL work — implementation, verification, review, council, and sweep. Implementation uses agent(agent='task'); verification uses agent(agent='reviewer'). Everything runs inside eval calls — no external subagent mechanisms. Handoff: an eval task agent implements, then a separate eval reviewer agent verifies; the same agent never plays both roles.
