...
3. **Implementation** — size-based, in precedence order: small > single-slice > full-DAG:
   - **Small** (≤3 files, ≤50 lines, low-risk) → LIGHT mode, K=1; if not low-risk, use single-slice or full-DAG (the standard loop — defaults to LIGHT mode and escalates to FULL per A7)
...
Design one monolithic DAG workflow for the task. Follow orchestration.md. Steps 1–4 below are the **execution-slice** path (work that may have real cross-step dependencies). A **parallel review/sweep** is an edge-less DAG instead — one composite node per lens (each node internally runs separate finder then verifier invocations), empty `depends_on`, no slicing: keep step 1 (scope) and step 4 (per-lens finder and verifier `agent` roles, plus `skills`/`quality_checks`), skip the slice/`depends_on` steps (2–3).
...
4. Per slice, assign: `agent` (eval agent type that RUNS this node — `task` for implementation, `reviewer` for review/verification — see [references/agents.md](references/agents.md)), `skills` (skill names to inject into the prompt), `quality_checks` (≥1 per slice, run in Step 2 node acceptance and again in Step 3) (a shell command or eval reviewer call; pass = exit 0/zero unaccepted real findings (to_act==0), fail = findings enter the convergence loop — see orchestration.md).

A **single-slice** task is another implementation path — the work is one cohesive change (a single file or a tightly related cluster) where slicing would be ceremony. Confirm it at the scope/scale gate: larger than trivial inline work (see Routing), yet with no real cross-step dependencies to record — decomposition would invent boundaries, not find them. The single-slice path omits decomposition only:
...
Single-slice is a shortcut in **decomposition** only, never in **verification**: one eval task agent implements, eval agents verify, and the same subagent invocation never performs both roles.

A **small task** — roughly ≤3 files and ≤50 lines of change — is a further implementation path: it shortens the whole loop, not just decomposition. The single-slice path still runs the full machinery (Step 2 eval verification, Step 3 K-round loop); a small task skips most of it:
...
Execute the DAG. At each decision, verify via the mode-appropriate adversarial pass — default LIGHT (single-vote batch verification, K=1); FULL when the user explicitly requests thorough or comprehensive coverage, or on sticky escalation (A7), where HIGH/CRITICAL get 2 additional votes beyond the batch verdict (total 3; see orchestration.md vote_count). Classify findings by Q2∧Q3 (never Q1 alone): treat confirmed-actionable findings as real, record borderline (low-confidence) ones, and drop only confidently non-actionable ones — a refuted claim with an actionable residual is kept. For a Q1-only `REFUTE_SCHEMA` vote, default `refuted` to `true` when unsure; for a structured `VERDICT_SCHEMA` verdict, encode uncertainty as `verification_confidence="low"` — never as claim status. Follow orchestration.md.
...
**Eval is the execution engine.** All delegated work — implementation, finder, verifier, review, council, and sweep — runs through eval `agent()` calls; no external subagent mechanisms. Orchestrator-side routing, searches, baselines, and shell gates remain direct. Implementation runs via `agent(agent='task')`, verification via `agent(agent='reviewer')`; the same subagent invocation never performs both roles.

1. Dispatch each ready node (deps satisfied) by role (per Step 2 delegation discipline): implementation via eval `agent(agent='task')`, planning/review/verification/council via eval `agent(agent='reviewer')`/`parallel()`; security-lens review via eval `agent(agent='security-reviewer')` (see agents.md). Inject upstream outputs and assigned skill names into the prompt. Run independent nodes concurrently. The same subagent invocation never performs both roles in one node.
2. Results that survive the eval verification are committed. Implementation nodes that fail ANY acceptance gate (compile/run, quality_checks, or review) retry with combined diagnostics/findings as feedback (up to 2 retries). Review/verification nodes are single-attempt — their verdicts are consumed as-is; a crashed or invalid review node blocks the Step 2→3 gate. EXCEPT that a batch verifier's id-completeness violation causes that batch to be retried exactly once (see orchestration.md **Batch verifier contract**); reviewer judgment failures remain single-attempt. Unconverged nodes fail — their dependents are skipped. Node acceptance = ALL gates pass (compile + quality_checks + to_act==0); retry limit is 2, with findings as feedback; see orchestration.md.

## Step 3 — Quality loop

Outside the DAG. Follow orchestration.md.

**Step 2→3 gate:** Enter Step 3 only after ALL required DAG nodes are accepted; if any required node FAILED after retries are exhausted, STOP — report the failed nodes and unresolved findings; never quality-check incomplete work. One-node paths (single-slice/small) pass when that node is accepted; review/sweep, when every finder returns structured findings output and every verifier actually dispatched for nonempty findings returns verdicts — a crashed node, or one returning no structured output, blocks Step 3 (a finder legitimately returning zero findings is valid, so a lens with no findings dispatches no verifier).

**Audit/review exit (replaces items 1–5 for review/sweep paths):** For review/sweep paths (zero implementation), canonical findings joined with their verifier verdicts are reported — items 1–5 do NOT apply. Instead, Step 2 performs the initial finder + verification pass (those are the finder/verifier nodes the gate above requires to return); Step 3 then loops over the canonical results. Round 1 runs the per-lens `quality_checks`, then ONE SYNTHESIS pass over the canonical findings/verdicts and the quality-check outputs — a reviewer call (schema: `SYNTHESIS_SCHEMA` — see orchestration.md) returning one structured result with two sections: integrated synthesis (cross-lens consistency/coverage) and uncovered gaps from the completeness critic (what remains unverified). Each subsequent round closes the prior critic's uncovered gaps: per-lens finder calls seeded by those gaps → batch-verification of their new findings (per the finder→verifier contract) → per-lens `quality_checks` → the SYNTHESIS reviewer call again, whose fresh gaps seed the next round. New findings per round are deduped against the cumulative SEEN set. Convergence = K consecutive rounds surfacing zero NEW findings AND zero uncovered gaps — nonempty `uncovered_gaps` seeds the next finder+verifier round and does not itself count as a dry round (K is mode-derived — LIGHT K=1, FULL K=2); if the user specified N rounds, run exactly N and stop. Findings are never remediated inline — acting on them starts a SEPARATE implementation DAG.
...
4. Repeat until **K consecutive rounds surface zero unaccepted real findings** (K is mode-derived — LIGHT K=1, FULL K=2). Borderlines and accepted findings don't reset K. If the user specified N rounds, run exactly N and stop, reporting unresolved findings.
