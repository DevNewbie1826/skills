...
- **Convergence termination (what resets K, and when to stop)** — only a **real, unaccepted** finding resets K: the test is `len(to_act) == 0` with `to_act = [f for f in real if f.id not in accepted]` — the SAME test A4 uses (never `len(real) == 0`); a round with zero actionable findings advances the K counter, and K consecutive such rounds converge (default K=2). Everything else leaves the counter untouched: **deduped** findings (already SEEN under a B14 canonical ID — re-finds are not new, see **Loop-until-dry**); **accepted** findings (A4 policy overlay — excluded from convergence by definition); **dropped** findings (`actionable_severity == "none"` AND `verification_confidence != "low"` — confidently nothing to do); and **borderline** findings (`verification_confidence == "low"` — REPORTED, never silently lost, but never a K reset, per the **Convergence rule**). These four are complements, not conflicts: B15 confirms that a verified actionable LOW is real and DOES reset K; this rule says a LOW that lands in dedupe/accept/drop/borderline does NOT. Separate stop condition — **external blocker park/stop**: when every remaining real finding is **blocked** by an external dependency the orchestrator cannot resolve (a physical device, an external API, or a user decision), do NOT keep spinning rounds. Park those findings with status `blocked`, record the external dependency each one waits on, and stop the loop; report them as blocked so work resumes where it stopped once the dependency clears. Convergence and blocking are distinct: K-dry means nothing new is being found; parked-blocked means the work is real but cannot proceed — neither abandons the findings, and the final report lists unresolved borderlines AND parked/blocked items.
- **Fix-regression resets K (a fix's side effects extend convergence)** — a fix applied in round N that surfaces a NEW real finding in round N+1 (a regression introduced by the fix) is a real finding like any other (unless accepted under A4), so it resets K; convergence holds only after K consecutive rounds with zero actionable findings (`len(to_act) == 0`) *following* the fix. A regression is an unexpected side effect, so extending convergence is the safe direction — do NOT treat it as expected churn under the **Convergence termination** rule.
- **Severity-weighted verification + light/full mode (mode-state contract, A6)** — verification weight is a per-task state, not a per-finding decision: the mode fixes, ONCE per task, how many verifiers judge a finding and what K convergence requires. **FULL mode (default)** — HIGH/CRITICAL findings get a 3–5-vote adversarial council (see **Adversarial verify**); MEDIUM/LOW findings get a single-vote verify (see **LOW severity is still verified**); convergence is K=2. **LIGHT mode** — explicitly selected for small, low-risk tasks: ALL findings, every severity included, get a single-vote verify and NO council; convergence is K=1 (one clean round suffices). **Eligibility**: LIGHT is a provisional pre-round selection — choose it when the task profile suggests low risk; if round 1 surfaces HIGH/CRITICAL findings, sticky-escalate to FULL (A7) — the effective mode persists across rounds (A7 implements the escalation; A6 selects the mode only). **Severity weights the VOTE, not the SKIP**: severity picks how many verifiers judge a finding (HIGH/CRITICAL → council, MEDIUM/LOW → single vote), never whether verification runs at all — every finding is still sent for verification in both modes, per the skip-gate rule. Vote counts by severity:
