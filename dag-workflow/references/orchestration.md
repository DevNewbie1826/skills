...
- **Shared context handoff** — Build ONE shared `local://` packet (see the `agent()` helper above) and reference it from every prompt; never paste copies, which waste tokens and drift. Include the user's goal, acceptance criteria/non-goals, exact baseline commit, changed-file allowlist, upstream decisions, and FULL VERBATIM, baseline-aware change view: `git diff <baseline>..<target>` for a committed range or `git show --patch <commit>` for one commit, plus `git diff HEAD`, `git status --short`, and task-relevant untracked TEXT files for the worktree overlay. Never substitute a bare commit pointer, bare `git diff HEAD` (empty after commit), or hand-summary: subagents otherwise assume "change = last commit" and guess goal/scope, wasting rounds. Keep only allowlisted files (exclude/redact secrets/binaries/oversized; `log()` each exclusion); never sweep the whole repo. If `local://` is unresolvable, keep one packet copy on a real filesystem path; inline into prompts only as last resort, but still generate once from the same allowlisted source.
...
- **Convergence rule + termination (what resets K, and when to stop)** — classify each finding from its verdict, not by feel. Three disjoint, schema-only buckets: **real** = `actionable_severity != "none"` AND `verification_confidence != "low"` (confirmed, actionable now); **borderline** = `verification_confidence == "low"` (the verifier is genuinely uncertain — REPORTED, never silently lost); **dropped** = `actionable_severity == "none"` AND `verification_confidence != "low"` (confidently nothing to do — log it, *no silent caps*). (Uncertainty is its OWN axis: `partial` stays a claim-status value, not a proxy for "unsure".) Convergence = **K consecutive rounds with zero actionable findings** — `len(to_act) == 0`, where `to_act = [f for f in real if f.id not in accepted]` — the SAME test A4 uses (never `len(real) == 0`; default K=2). A round with zero actionable findings advances the K counter; only a **real, unaccepted** finding resets it. Everything else leaves the counter untouched: **deduped** findings (already SEEN under a B14 canonical ID — re-finds are not new, see **Loop-until-dry**); **accepted** findings (A4 policy overlay — excluded from convergence by definition); **dropped** findings (`actionable_severity == "none"` AND `verification_confidence != "low"` — confidently nothing to do); and **borderline** findings (`verification_confidence == "low"` — REPORTED, never silently lost, but never a K reset). These four are complements, not conflicts: B15 confirms that a verified actionable LOW is real and DOES reset K; a LOW that lands in dedupe/accept/drop/borderline does NOT. The final report lists every unresolved borderline. If the user set N rounds, run exactly N regardless of early convergence, and report what's left. Separate stop condition — **external blocker park/stop**: when every remaining real finding is **blocked** by an external dependency the orchestrator cannot resolve (a physical device, an external API, or a user decision), do NOT keep spinning rounds. Park those findings with status `blocked`, record the external dependency each one waits on, and stop the loop; report them as blocked so work resumes where it stopped once the dependency clears. Convergence and blocking are distinct: K-dry means nothing new is being found; parked-blocked means the work is real but cannot proceed — neither abandons the findings, and the final report lists unresolved borderlines AND parked/blocked items.
