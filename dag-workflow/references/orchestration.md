...
- **Parallel writes (fixes/migrations)** — dispatch each implementation slice via eval `agent(agent='task')`. Pin the shared interface (a widened type, a header) as its OWN preemptive slice — land it first, confirm it compiles — THEN fan out the consumer slices as concurrent eval `agent(agent='task')` calls; files that share a contract line stay in ONE slice. Concurrent writers must own DISJOINT files (no two agents edit the same file in the same round); shared paths or contract lines are SERIALIZED — the preemptive interface slice lands and compiles first, then consumer slices fan out. After all writers return, verify cross-slice consistency, then run the integrated eval cross-slice review pass (`agent(agent='reviewer')`/`parallel()`). Failure recovery: if a writer's changes break compilation or cross-slice consistency, revert ONLY that writer's delta — snapshot each writer's touched files (git stash create or cp before dispatch) and restore just those files on failure, preserving all other workspace state. Retry the writer with feedback (up to 2 retries per node acceptance).
