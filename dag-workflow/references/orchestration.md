...
- **Node acceptance** — Accept/commit an implementation node only if (1) task-agent output compiles/runs, (2) `quality_checks` pass (see **quality_checks contract**), and (3) eval finds `to_act == 0` (see **Convergence rule + termination**). On failure, send session task agent feedback: compile/run diagnostics; failed shell checks as `{id: 'qc:<name>', file: '<quality-check>', title: 'quality_check failed: <command>', severity: 'high', detail: 'exit: <code>, stderr: <output>'}` (`high` is conservative initial severity pending Q2/Q3 verification); and review findings (`id`/`severity`/`detail`/`reason`). After 2 failed retries, fail node, skip dependents, and report unresolved failures. Never retry review/verification nodes; consume their verdict.
</execution>
</system-notice>
