...
- **Parallel writes (fixes/migrations)** — dispatch each implementation slice as a separate eval `agent(agent='task')` (implementation runs inside eval — per Step 2's eval execution rule). Pin the shared interface (a widened type, a header) as its OWN preemptive slice — land it first via eval `agent(agent='task')` (land = apply/merge the slice's isolated result to the main workspace via `apply=True`), confirm it compiles — THEN fan out the consumer slices as concurrent eval `agent(agent='task')` agents with isolated workspace enabled (Python: `isolated=True`, JavaScript: `{ isolated: true }`); files that share a contract line stay in ONE slice. Concurrent implementation agents MUST use ISOLATED workspaces — dispatch each writer via `agent(prompt, agent='task', isolated=True)`: isolated gives each writer its own workspace; changes auto-land (apply defaults to true) when the writer returns. Concurrent eval task agents must own DISJOINT files (no two agents edit the same file in the same round); shared paths or contract lines are SERIALIZED — the preemptive interface slice lands and compiles first, then consumer slices fan out. Failure recovery: if a writer's changes break compilation or cross-slice consistency, revert that writer's files via git checkout and retry the writer with feedback (up to 2 retries per the node acceptance contract). After all slices land and their results are applied/merged, verify with `git diff`/`git status` plus the workspace stability guard — capture the stability-guard baseline AFTER all accepted implementation writes have been applied/merged, not before dispatch. Close the residual gap with an integrated eval cross-slice review pass (eval `agent(agent='reviewer', …)`/`parallel()`) after all slices land.
